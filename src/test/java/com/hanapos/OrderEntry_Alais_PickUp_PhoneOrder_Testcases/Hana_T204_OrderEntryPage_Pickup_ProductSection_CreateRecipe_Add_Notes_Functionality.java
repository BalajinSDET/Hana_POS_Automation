...
public class Hana_T204_OrderEntryPage_Pickup_ProductSection_CreateRecipe_Add_Notes_Functionality extends TestBaseClass {
	...
	public void Validate_Hana_T204_OrderEntryPage_Pickup_ProductSection_CreateRecipe_Add_Notes_Functionality_Test() {
		SoftAssert softassert = new SoftAssert();
		logger.info("**** Starting  Validate_Hana_T204_OrderEntryPage_Pickup_ProductSection_CreateRecipe_Add_Notes_Functionality_Test ****");
		...
		try {
			...
			softassert.assertEquals(phoneorder.get_entered_privatenotes_CreateRecipe_Popup(),"Recipe Test Automation Private Notes","Test Step - 15 - Entered Private notes is not displayed on create recipe popup add notes section");
			...
			softassert.assertEquals(phoneorder.get_entered_publicnotes_CreateRecipe_Popup(),"Recipe Test Automation Public Notes","Test Step - 16 - Entered Public notes is not displayed on create recipe popup add notes section");
			
			// Test Step - 17
			phoneorder.Click_PhotoTab_AddNotesSection();
			delayWithGivenTime(2000);
			phoneorder.Upload_photo_AddFile_Button_OnCreateRecipePopup("roses red.jpg");
			softassert.assertTrue(phoneorder.Verify_ImageIsDisplayed_After_UploadTheFile(),"Test Step - 17 - Uploaded photo is not displayed on create recipe popup add notes section");
			...
			phoneorder.Click_ProductDetails_Row1_Recipe();
			delayWithGivenTime(2000);
			softassert.assertTrue(phoneorder.Verify_CreateRecipe_Popup(),"Test Step - 19 - Create Recipe popup is not displayed");

			// Test Step - 20			
			phoneorder.Click_Private_NotesTab_AddNotesSection();
			delayWithGivenTime(2000);
			softassert.assertEquals(phoneorder.get_entered_privatenotes_CreateRecipe_Popup(),"Recipe Test Automation Private Notes","Test Step - 20 - Entered Private notes is not displayed after the save the recipe and came back to view once again on create recipe popup add notes section");
			phoneorder.Clear_PrivateNotes_CreateRecipe_Popup(); // to avoid junk data
			
			// Test Step - 21		
			phoneorder.Click_Public_NotesTab_AddNotesSection();
			delayWithGivenTime(2000);
			softassert.assertEquals(phoneorder.get_entered_publicnotes_CreateRecipe_Popup(),"Recipe Test Automation Public Notes","Test Step - 21 - Entered Public notes is not displayed after the save the recipe and came back to view once again on create recipe popup add notes section");
			phoneorder.Clear_PublicNotes_CreateRecipe_Popup(); // to avoid junk data
			
			// Test Step - 22
			phoneorder.Click_PhotoTab_AddNotesSection();
			delayWithGivenTime(2000);
			softassert.assertTrue(phoneorder.Verify_ImageIsDisplayed_After_UploadTheFile(),"Test Step - 17 - Uploaded photo is not displayed after the save the recipe and came back to view once again on create recipe popup add notes section");			
