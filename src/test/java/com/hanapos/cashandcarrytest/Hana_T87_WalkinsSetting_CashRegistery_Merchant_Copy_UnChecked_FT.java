...
public class Hana_T87_WalkinsSetting_CashRegistery_Merchant_Copy_UnChecked_FT extends TestBaseClass {
	...
	/*
	 * HANA-T87 - 1 to 15 - Verify Cash and Carry Page - Walkins setting - Cash Registery - Merchant Copy with UnChecked- Functionality
	...
	public void Validate_Hana_T87_WalkinsSetting_CashRegistery_Merchant_Copy_UnChecked_FT() {
		...
		try {
			...
			// Pre-requisite - Go to walkins setting --> edit the cash registry (Cash Register unchecked FT ) --> 
					...
					delayWithGivenTime(4000);
					// Verify merchant copy checkbox as unchecked 
					if(cashandcarry.merchantCopyCheckBoxonAddNewpopUPIsSelected()==false) {
					softassert.assertFalse(cashandcarry.merchantCopyCheckBoxonAddNewpopUPIsSelected());
					}else {
						softassert.fail("Merchant copy checkbox is not getting unchecked");
					}
