package com.hanapos.loginpagetest;
...
public class Hana_T153_Pickup_RecipientSection_AttentionFieldLookup_FT extends TestBaseClass {
	...
	public void Validate_Hana_T153_Pickup_RecipientSection_AttentionFieldLookup_Functionality_Test() {
		...
		try {
			...
			softassert.assertEquals(phoneorder.getReciPhone(),"956-444-0756","Test Step - 7 - Recipient phone number is not matched with customer phonenumber 1 field on phone order page recipient section");
			softassert.assertEquals(phoneorder.getRecipientPhone2OnPhoneOrderPage(),"956-444-0756","Test Step - 7 - Recipient alt phone number is not matched with customer phonenumber2 on phone order page recipient section");
			...
			// Test Step - 9
			phoneorder.EnterRecipientAttention("Chicago");
			...
			// Test Step - 11
			phoneorder.EnterRecipientAttention("Washington Square Park");
			phoneorder.SearchAndSelectReciAttention("Washington Square Park New York");
			delayWithGivenTime(2000);
			...
			softassert.assertEquals(phoneorder.getReciAddress1(),"Washington Square","Test Step - 11 - Recipient attention is not matched with Attention field on phone order page recipient section");
			softassert.assertEquals(phoneorder.getReciZipcode(),"10012","Test Step - 11 - Recipient address 2 is not matched with customer address 2 on phone order page recipient section");
			softassert.assertEquals(phoneorder.getReciCity(),"New York","Test Step - 11 - Recipient city is not matched with customer city on phone order page recipient section");
			softassert.assertEquals(phoneorder.getRecipientState(),"NY","Test Step - 11 - Recipient phone number is not matched with customer phonenumber 1 field on phone order page recipient section");
			softassert.assertEquals(phoneorder.getReciPhone(),"956-444-0756","Test Step - 11 - Recipient phone number is not matched with customer phonenumber 1 field on phone order page recipient section");
		
			// Test Step - 12
			delayWithGivenTime(1000);
			phoneorder.EnterRecipientAttention("Washington");
			delayWithGivenTime(1000);
			softassert.assertEquals(phoneorder.get_RecipientAttention(),"Washington","Test Step - 12 - Recipient attention is not matched with Attention field on phone order page recipient section");
			
			// Test Step - 13
			delayWithGivenTime(1000);
			phoneorder.EnterRecipientAttention("Chicago Midway International Airport");
			phoneorder.SearchAndSelectReciAttention("Chicago Midway International Airport");
			delayWithGivenTime(2000);
			...
			softassert.assertEquals(phoneorder.getReciPhone(),"956-444-0756","Test Step - 13 - Recipient phone number is not matched with customer phonenumber 1 field on phone order page recipient section");
